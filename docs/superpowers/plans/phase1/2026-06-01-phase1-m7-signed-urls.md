...
- **M10 master-key re-wrap coupling (documented, not M7 code):** `signing_keys.wrapped_key` is master-wrapped, so M10's `rotate-master` re-wrap worker must walk `signing_keys` (state `active` + within-grace `retired`) as well as the blob DEKs, or rotation orphans the signing keys. This was raised in review and is already recorded in `ENCRYPTION_ENVELOPE.md` § "Rotation procedure" step 4, the master plan M10 deliverables/exit, the `signing_keys` schema comment, and design § "Risks and notes" / reconciliation #6 — no action inside M7 beyond keeping those notes intact.
