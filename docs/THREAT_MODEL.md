...
| Multi-master high availability (independent coordinators with divergent writable databases accepting concurrent writes) | Forever out of scope. Independent writable masters would require Byzantine-tolerant or logical-replication conflict resolution that is unacceptable for crypto-shred, legal holds, and assignment ordering. **Note:** this is distinct from the proposed post-1.0 Phase 6 (`T1.27`), which is *multi-coordinator, single-authority* HA — several serving replicas over **one** strongly-consistent authority with a fenced leader — and does **not** reintroduce concurrent-write consensus. |
| Cold-standby failover for the coordinator | Not shipped today, but compatible with the architecture. An operator may run a hot-spare host with Postgres streaming replication and the same `NOVA_MASTER_KEY` versions loaded; failover is manual. See `docs/recipes/COLD_STANDBY.md`. Automating and hardening this into mechanical-fenced failover is the proposed post-1.0 Phase 6 (see `ROADMAP.md`). |
