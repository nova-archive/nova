...
> **Amended by P2-M7 (2026-07-01) — implemented.** **Drain semantics**
> (D-M7-6b/6c/6f): `nodes.draining_at IS NOT NULL` is the authoritative
> voluntary-departure marker, distinct from `placement_weight = 0` (a
> throttle that still counts). A draining node is EXCLUDED from the safety
> counts (`healthy_acked`, `sourceable_acked`, `CountSourceableHolders` —
> commit/prune/read viability) and from placement candidacy, but REMAINS an
> eligible, deprioritized read/repair source while live (prepended
> `draining_at IS NOT NULL` sort key; repair source of last resort).
> **Drain debt** = CIDs acked on the draining node whose count of acked,
> live, sync-current, non-draining holders is below `target_count`;
> pending reservations never reduce it (`CountDrainPendingCIDs` /
