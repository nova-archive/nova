...
| **P2-M4.1** ⏳ | **Storage/read redirect — required before P2-M5.** Carries the P2-M2.1 amendment that M4 deliberately deferred: donor-backed reads on a coordinator cache miss (select live `acked` holder → fetch ciphertext → verify → decrypt → serve → bounded re-cache), `require_replication_quorum_before_commit` (an upload is not durable/visible until a commit quorum of donor acks), origin/staging **pruning** with a `prune_safety_floor` (never below N live donor holders), `coordinator_storage_mode` (`origin_copy`/`bounded_cache`/`transient`) + the bounded hot cache, and transform re-fetch from donors. **The P2-M2.1 storage/read target is NOT fulfilled until P2-M4.1 lands**, and **P2-M5 must be designed assuming the coordinator origin copy is not guaranteed.** The read model removes the *coordinator-local-origin* requirement, not the coordinator from the read path — the coordinator stays the sole decrypt/serve point (`T1.26`); donors never serve plaintext. |
