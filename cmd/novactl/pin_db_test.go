...
func TestPinAssignListUnpin(t *testing.T) {
	...
	node := uuid.New()
	if _, err := pool.Exec(ctx, `INSERT INTO nodes (id,nebula_cert_fingerprint,federation_cert_fingerprint,capacity_bytes,bandwidth_budget_bytes_per_day,advertised_capabilities,effective_capabilities) VALUES ($1,$2,$3,0,0,ARRAY['blob-transfer/v1'],ARRAY['blob-transfer/v1'])`,
